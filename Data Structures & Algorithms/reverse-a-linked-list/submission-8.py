...
class Solution:
    def reverseList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        ...
        # Check base case or if list is empty
        if cur == None:
            return None
        
        # Keep returning hte new head up the Call Stack
        newHead = cur

        if cur.next != None:
            newHead = self.reverseList(cur.next)

            # This reverses the linke between the next and current node
            cur.next.next = cur
        ...
        """
        Time O(n)
        Space O(n) since we have to put a functionin the call stack for every element in the LL
        """
