...
""" Create CONSTRUCTOR Class so that you can Create a New Node with Null Pointers"""
class ListNode:
    def __init__(self, val):
...
class MyLinkedList:
    ...
    def get(self, index: int) -> int:
        ...
        while cur and index > 0:
            # Iterate through LL and get to the Current Value
            cur = cur.next
            index -= 1


        # Need Cur Validation so we DO NOT End on a Dummmy Node
        if cur != self.right and index == 0:  
            return cur.val
    ...
    def addAtHead(self, val: int) -> None:
        """Dummy Values are Needed for This Section"""
        # Store current and next value into pointers for the swap below
        prev = self.left
        nextNode = self.left.next

        # Create New Node Reset pointers for existing elements to the New Node
        newNode = ListNode(val)
        prev.next = newNode
        nextNode.prev = newNode

        # Set Pointers for New Inserted Val
    ...
    def addAtTail(self, val: int) -> None:
        ...
        # Reset pointers for existing elements
        newNode = ListNode(val)
        prev.next = newNode
        nextNode.prev = newNode

        # Reset Pointers for New Inserted Val
        newNode.prev = prev
        newNode.next = nextNode
    ...
    def addAtIndex(self, index: int, val: int) -> None:
        ...
        # Arrive to target index
        while cur and index > 0:
            cur = cur.next
            index -= 1

        # Only Add Node if we are In Bounds and if the 
        # Target Index has been Reached
        if index == 0:
            """ Store Varialbles for Use Later for Updating"""
            prev = cur.prev
            nextNode = cur

            # Reset Existing Pointers to New Node
            newNode = ListNode(val)
            prev.next = newNode
            nextNode.prev = newNode
    ...
    def deleteAtIndex(self, index: int) -> None:
        ...
        """ Need the curr part of the statement because 
        DO NOT want to End on a DUMMY NODE"""
        if cur != self.right and index == 0:
            # Store pointers to Cur and Previous
            prev = cur.prev
            nextNode = cur.next

            # Reset pointers for nodes that are staying
            prev.next = nextNode
            nextNode.prev = prev

            # Disconnect pinters from deletion node
            cur.next = None
            cur.prev = None
